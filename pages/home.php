<?php
require_once __DIR__ . '/../logic/home_logic.php';  // โหลดไฟล์ logic ด้านหลัง (เช่น การดึงข้อมูลฐานข้อมูล และฟังก์ชันต่างๆ)
?>
...
<div id="map-content">
  ...
  <div id="map-overlay">
    ...
    <?php foreach ($stations as $row): ?>
      ...
      ?>
      <div class="marker <?= $colorClass ?>" style="top: <?= $pos['name']['top'] ?>px; left: <?= $pos['name']['left'] ?>px; font-size:7px;">
        <?= htmlspecialchars($name) ?>
      </div>
      <div class="marker <?= $colorClass ?>" style="top: <?= $pos['water']['top'] ?>px; left: <?= $pos['water']['left'] ?>px; font-size: 6px;">
    ...
    <?php foreach ($stations as $row): ?>
      ...
      ?>
      <!-- แสดงค่าปริมาณน้ำ -->
      <div class="marker marker-dam" style="position: absolute; top: <?= $pos['top'] ?>px; left: <?= $pos['left'] ?>px; font-size : 8px; ">
  ...
  <div class="data-row">
    ...
    <span class="col-current"><?= number_format($current) ?> (<?= number_format($currentPercent, 0) ?>%)</span>
    <span class="col-inuse"><?= number_format($inuse) ?> (<?= number_format($inusePercent, 0) ?>%)</span>
  </div>
</div>
